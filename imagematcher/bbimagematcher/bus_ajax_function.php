...
$bus_obj=new Bus();
			...
			if($service=="bus_view_datatable_function" || $service==24)
			{
				// storing  request (ie, get/post) global array to a variable
				$requestData= $_REQUEST;
				$totalData = $bus_obj->countBus("1","1");
				$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.

				// Response Column Name Specify
				$columns = array(
				// datatable column index  => database column name
					0=>"id",1=>"bus_no",2=>"latitude",3=>"longitude",
				);
				// getting total number records without any search
				if( !empty($requestData['search']['value']) ) {
					"id LIKE %".$requestData['search']['value']."%  OR bus_no LIKE %".$requestData['search']['value']."%  OR latitude LIKE %".$requestData['search']['value']."% ";
				}
				$totalFiltered = $bus_obj->countBus("isDelete","0");
				...
				$projection=$columns;
				$data_row=$bus_obj->view($condition,$projection,$order_by,$limit);
				$json_data = array(

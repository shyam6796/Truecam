...
class User extends Functions
{
	...
	public $unique_column="email";
	// Public Varibale
	public $id='';public $name='';public $email='';public $phone='';public $bdate='';public $address='';public $city='';public $state='';public $country='';
	public $valid_keys=array("id","name","email","phone","bdate","address","city","state","country");
		...
		if($this->rights['update_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail)&& $primary_key!="")
				{
					...
					if($count>=1)
					{	
						$count=$this->duplicateUser($this->unique_column,$unique_key,$primary_key);
						if($count<=0)
						{
	...
	{		
		$countUser = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
		return $countUser;	
	}
	function duplicateUser($key,$value,$primary_key)
	{		
		$countUser = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
		return $countUser;	
	}

...
$bus stop_obj=new Bus stop();
			...
			if($service=="bus stop_view_datatable_function" || $service==25)
			{
				// storing  request (ie, get/post) global array to a variable
				$requestData= $_REQUEST;
				$totalData = $bus stop_obj->countBus stop("1","1");
				$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.

				// Response Column Name Specify
				$columns = array(
				// datatable column index  => database column name
					0=>"id",1=>"bus_stop_title",2=>"bus_stop_address",3=>"bus_stop_locality",
				);
				// getting total number records without any search
				if( !empty($requestData['search']['value']) ) {
					"id LIKE %".$requestData['search']['value']."%  OR bus_stop_title LIKE %".$requestData['search']['value']."%  OR bus_stop_address LIKE %".$requestData['search']['value']."% ";
				}
				$totalFiltered = $bus stop_obj->countBus stop("isDelete","0");
				...
				$projection=$columns;
				$data_row=$bus stop_obj->view($condition,$projection,$order_by,$limit);
				$json_data = array(

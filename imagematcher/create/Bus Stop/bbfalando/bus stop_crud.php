...
$page_hierarchy=array(array("link"=>"dashboard.php","title"=>"dashboard"),array("link"=>"bus_stop_manage.php","title"=>"Manage Bus stop"),array("link"=>"bus_stop_crud.php","title"=>"Add/Edit Bus stop "));
$success_msg=array();
$error_msg=array();
// PAGE DECLARATION
...
$params['bus_stop_longitude']=trim($db->clean($_REQUEST['bus_stop_longitude']));
	...
	if($mode=='a')
	{
		...
		$reply=$bus stop_obj->insert($params,array("key"=>"bus_stop_title","value"=>$params['bus_stop_title']));
		if($reply['ack']==1)
		{
	...
	else if($mode=='e')
	{
		...
		$reply=$bus stop_obj->update($params,$params['bus_stop_title'],$params['id']);
		if($reply['ack']==1)
		{
			$success_msg[]=$reply['ack_msg'];
		}
	...
	{
		//
		$reply=$bus stop_obj->delete(array("key"=>"id","value"=>$params['id']));
		if($reply['ack']==1)
		{

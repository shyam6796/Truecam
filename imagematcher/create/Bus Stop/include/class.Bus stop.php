<?php
class Busstop extends Functions
{
	public $detail=array();
	public $db,$rights;
		...
		if($this->rights['insert_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail))
				{
					...
					$count=$this->countBusstop($dup_check_array['key'],$dup_check_array['value']);
					else
					$count=0;	
					if($count<=0)
					{
		...
		if($this->rights['update_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail)&& $primary_key!="")
				{
					...
					$count=$this->countBusstop($this->primary_column,$primary_key);
					if($count>=1)
					{	
						$count=$this->duplicateBusstop($this->unique_column,$unique_key,$primary_key);
						if($count<=0)
						{
	...
	function delete($delete_array)
	{
		if($this->rights['delete_flag']==1)
		{
			// Hey Its Really Delete Your Row i am not kidding :| !!
				...
				$count=$this->countBusstop($delete_array['key'],$delete_array['value']);
				if($count>=1)
				{
	...
	function  active($active_array)
	{
		if($this->rights['update_flag']==1)
		{
				...
				$count=$this->countBusstop($active_array['key'],$active_array['value']);
				if($count>=1)
				{
					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
					$reply=array("ack"=>1,"developer_msg"=>"Bus stop deail status updated successfully!!","ack_msg"=>"Bus stop status updated successfully!!");
					return $reply;
				}
				else
				{

<?php
class Bus extends Functions
{
	public $detail=array();
	public $db,$rights;
	public $ctable="bus";
	public $primary_column="id";
	public $unique_column="bus_no";
	// Public Varibale
	public $id='';public $bus_no='';public $latitude='';public $longitude='';public $='';public $='';
	public $valid_keys=array("id","bus_no","latitude","longitude","","");
	...
	function view($where="",$required_columns=array(),$orderby="",$limit="")
	{
		...
		if($this->rights['view_flag']==1)
		{
			...
			if($result_r)
			{
				...
				$reply=array("ack"=>1,"developer_msg"=>"Bus deail fetched successfully!!","ack_msg"=>"Bus deail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Bus detail can’t be fetch!","ack_msg"=>"Bus detail can’t be fetch!");
				return $reply;
			}
		}
		else
		...
		if($this->rights['insert_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail))
				{
					...
					$count=$this->countBus($dup_check_array['key'],$dup_check_array['value']);
					else
					$count=0;	
					if($count<=0)
					{
						...
						if($inserted_id!=0)
						{
							$reply=array("ack"=>1,"developer_msg"=>"Bus detail inserted successfully!!","ack_msg"=>"Bus detail inserted successfully!!");
							return $reply;
						}
						else
						{
							$reply=array("ack"=>0,"developer_msg"=>"Bus deail can't be insert!!","ack_msg"=>"Bus deail can't be insert!!");
							return $reply;
						}
					}
					else
				...
				if(!empty($detail)&& $primary_key!="")
				{
					...
					{	
						...
						if($count<=0)
						{
							...
							if($isUpdated)
							{
								$reply=array("ack"=>1,"developer_msg"=>"Bus detail updated successfully!!","ack_msg"=>"Bus detail updated successfully!!");
								return $reply;
							}
							else
							{
								$reply=array("ack"=>0,"developer_msg"=>"Bus detail can’t be updated!!","ack_msg"=>"Bus detail can’t be updated!!");
								return $reply;
							}
						}
						else
	...
	function delete($delete_array)
	{
		if($this->rights['delete_flag']==1)
		{
			...
			if(!empty($dup_check_array))
			{
				$count=$this->countBus($delete_array['key'],$delete_array['value']);
				if($count>=1)
				{
					$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value']);
					$reply=array("ack"=>1,"developer_msg"=>"Bus deail deleted successfully!!","ack_msg"=>"Bus deail deleted successfully!!");
					return $reply;
				}
				else
				{
	...
	function countBus($key,$value)
	{		
		$countBus = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
		return $countBus;	
	}
